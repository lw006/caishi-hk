...
class Pub
{
    ...
    public function checkLogin()
    {
        if ($this->request->isAjax() && $this->request->isPost()) {
            ...
            if (null === $auth_info) {
                ...
                (new model\AdminUser())->allowField(true)->save($update,['id' => $auth_info['id']]);

                // 记录登录日志
                $log['admin_id'] = $auth_info['id'];
                ...
                (new model\AdminLoginLog($log))->save();
                // 缓存访问权限
                \Rbac::saveAccessList();
    ...
    public function password()
    {
        ...
        if ($this->request->isPost()) {
            ...
            // 写入新密码
            if (false === (new model\AdminUser())->updatePassword(UID, $data['password'])) {
